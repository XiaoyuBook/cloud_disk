...
std::unique_ptr<cloud_disk_service::SRPCClient> get_srpc_client() {
    return std::make_unique<cloud_disk_service::SRPCClient>(RPC_IP, RPC_PORT);
}

// rpc服务端确认
void consul_callback(WFHttpTask *task) {

    series_of(task)->set_context((void*)socket);
...
void CloudiskServer::register_callback(const HttpReq* req, HttpResp *resp, SeriesWork *series) {
    ...
    // 注册中心consul部分
    rpc_socket socket;
    WFHttpTask *consul_task = WFTaskFactory::create_http_task(
        CONSUL_CHECK_URL, 3, MAX_RETRY, [series, username, password, resp](WFHttpTask* task){
            const void* body;
            size_t body_len;
            ...
            if(data.empty()) {
                resp->set_status(503);
                resp->String("Service unavailable: no user_service instance.");
                return;
            }
            json service_info  = data[0]["Service"];
            string ip = service_info["Address"];
            int port = service_info["Port"];


            // 微服务逻辑
            using namespace srpc;
            cloud_disk_service::SRPCClient client(ip.c_str(), port);
...
void CloudiskServer::login_callback(const HttpReq* req, HttpResp *resp,SeriesWork *series){
    ...
    // consul注册中心部分
    WFHttpTask *consul_task = WFTaskFactory::create_http_task(
        CONSUL_CHECK_URL, 3, MAX_RETRY,
        [resp, username, password, series](WFHttpTask *task) {
            const void *body;
            size_t body_len;
            task->get_resp()->get_parsed_body(&body, &body_len);
            std::string content(static_cast<const char *>(body), body_len);
            ...
            try {
                ...
                resp->set_status(500);
                resp->String("Failed to parse Consul response");
                return;
            }
            ...
            json service_info = data[0]["Service"];
            std::string ip = service_info["Address"];
            int port = service_info["Port"];
    ...
    WFMySQLTask *sql_task = WFTaskFactory::create_mysql_task(MYSQL_URL, MAX_RETRY,
        [req, resp, series,username](WFMySQLTask *sql_task) {
            ...
            for (const auto& [_, file] : form) {
                ...
                RabbitMQ r(RABBITMQ_CONFIG_PATH);
