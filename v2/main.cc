        ...
        [req, resp, series,username](WFMySQLTask *sql_task) {
            ...
            for (const auto& [_, file] : form) {
                ...
                if(!outcome.isSuccess()) {
                    cout << "PutObject fail" << ",code:" << outcome.error().Code() << 
                    ",message: " << outcome.error().Message() << ",requestId: " << outcome.error().RequestId() << endl;
                    resp->set_status(500);
                    resp->String("OSS Upload failed: " + outcome.error().Message());
                    ShutdownSdk();
                    return;
                }
                ShutdownSdk();
